...
# if st.button("Recommend"):
#     try:
#         idx = df[df["song_name"].str.strip().str.lower() == user_input.strip().lower()].index[0]
...
#         with col2:
#             st.write("**Song:**", df.iloc[idx]['song_name'])
#             st.write("**Artist:**", df.iloc[idx]['artist'])  # ✅ Fixed
#     except Exception as e:
#         st.error(f"Song not found. Error: {e}")  # ✅ Error dikhega ab
#         st.stop()
